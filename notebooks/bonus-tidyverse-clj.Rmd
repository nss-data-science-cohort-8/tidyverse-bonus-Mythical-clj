...
grades <- all_students |> 
  group_by(grade) |> 
  filter(subject == 'Algebra I' & grade != 'Missing Grade' & grade != 'All Grades') |> 
...
all_schools <-  all_students |> 
  filter(system_name == 'Rutherford County' & grepl('High School', school_name) & subject == 'Algebra I' & grade == 'All Grades') |> 
  group_by(school_name, grade) |> 
  select(school_name, grade, pct_below_bsc, pct_bsc, pct_prof, pct_adv)

dfm <- all_schools |> 
  pivot_longer(c(pct_below_bsc, pct_bsc, pct_prof, pct_adv))
...
ggplot(dfm, aes(x=school_name, y=value, fill=name)) +
