...
county_func <- function(df, county){
  ...
  ggplot(dff, aes(x=school_name, y=value, fill=name)) +
  geom_col(position = "dodge", stat = "identity") +
    scale_fill_manual(values = c("#6C2CDB",
                               "#86525B",
                               "#5C5740",
  ...
  ggsave(file=glue("../images/{county} Bar.png"), width=6, height=4)
  }
```
```{r}
county_func(df = all_students, county="Williamson County")
```
